...
def start_process(filename):
    ...
    if ext == ".py":
        cmd = [sys.executable, "-u", str(fpath)]
    ...
    def reader():
        try:
            for line in iter(proc.stdout.readline, ""):
                if not line:
                    break
                logs.append(f"[{time.strftime('%H:%M:%S')}] {line.rstrip()}")
...
def run_install(command):
    ...
    # Use python -m pip for reliability + unbuffered output
    if parts[0] in ("pip", "pip3"):
        cmd = [sys.executable, "-u", "-m", "pip", "install"] + parts[2:]
    else:
        cmd = parts  # npm install ...
    
    logs = INSTALL_LOGS.setdefault(WORKSPACE, deque(maxlen=2000))
    logs.clear()  # fresh install logs each time
    logs.append(f"[INSTALL] $ {' '.join(cmd)}")
    logs.append(f"[INSTALL] WORKDIR: {USER_DIR}")
    logs.append("[INSTALL] STARTING...")
    cwd = str(USER_DIR)
    ...
    def worker():
        try:
            ...
            logs.append(f"[INSTALL-ERROR] RUNTIME NOT FOUND: {e}")
        except Exception as e:
...
@app.route("/install/logs")
def install_logs_api():
    return jsonify({"logs": get_install_logs()})
...
<div class="term-body" id="installOutput" style="max-height:200px">— READY —</div>
</div>
</div>
</div>
...
// Install logs - separate real-time refresh
function refreshInstallLogs(){
    fetch('/install/logs').then(r=>r.json()).then(d=>{
...
function installModule(){
    ...
    if(!cmd)return;
    document.getElementById('installOutput').textContent='[INSTALL] STARTING...';
    ...
    .then(r=>r.json()).then(d=>{
        ...
        // Immediately start refreshing install logs
        refreshInstallLogs();
    });
}

setInterval(refreshLogs,4000);
setInterval(refreshInstallLogs,2000);
refreshLogs();
refreshInstallLogs();
</script>
</body>
